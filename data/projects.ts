...
export const projects: Project[] = [
  ...
  },
  {
    id: 5,
    title: "CineView",
    subtitle: "AI-Powered Movie Discovery App",
    description:
      "A modern, responsive movie discovery web application built with Angular. Uses the TMDB API for movie data and integrates with the Google Gemini AI API for smart 'vibe' searching and content summarization.",
    longDescription:
      "CineView is a feature-rich, standalone component-based application built with Angular 17+. It showcases modern state management using Signals and computed values. The app provides a seamless movie browsing experience, pulling data from the TMDB API. Key features include a reactive search bar using RxJS (debounceTime, switchMap), a 'Vibe Search' powered by the Google Gemini API, and an AI-generated summary feature on movie detail pages. The application also includes a complete mock authentication flow with `localStorage` persistence and protected routes using functional guards.",
    image: "/imgs/Project5/1.png",
    images: [
      "/imgs/Project5/2.png",
      "/imgs/Project5/3.png",
      "/imgs/Project5/4.png",
      ...
      "/imgs/Project5/8.png",
      "/imgs/Project5/9.png",
      "/imgs/Project5/10.png",
      "/imgs/Project5/11.png",
      "/imgs/Project5/14.png",
      "/imgs/Project5/15.png",
      "/imgs/Project5/16.png",
    ],
    links: {
      github: "https://github.com/hussein1574/cineview",
      live: "https://cine-view-puce.vercel.app/",
    },
    tags: [
      "Angular",
      "TypeScript",
    ...
    technologies: {
      frontend: [
        ...
        "RxJS (debounceTime, switchMap, forkJoin, catchError)",
        "TailwindCSS",
        "TypeScript",
        "HTML5 & CSS3",
      ],
    ...
    features: [
      "Browse & Discover Trending Movies",
      "View Detailed Movie Pages (Synopsis, Runtime, Genres)",
      "Reactive Search Bar with RxJS (debounceTime, switchMap)",
      "AI 'Vibe' Search using Google Gemini API",
      "AI-Generated Movie Summaries on detail pages (Gemini API)",
      "Mock Authentication (Login/Logout) with localStorage persistence",
      "Protected Routes (Functional Route Guards for Watchlist)",
      "Public-Only Routes (Functional Route Guard for Login)",
      ...
      "Custom `MinutesToHoursPipe` for runtime formatting",
      "Centralized API Key Handling via HTTP Interceptor",
    ],
    challenges: [
      "Managing application-wide state reactively using Angular's new Signal-based architecture.",
      "Integrating two distinct external APIs (TMDB and Gemini) and managing their different API key requirements securely and efficiently.",
      "Implementing a smooth, responsive real-time search bar that provides instant feedback without sending excessive API requests.",
      "Orchestrating multiple asynchronous API calls for the 'Vibe Search' feature, where an AI-generated list of titles must be individually searched on another API.",
      "Securing application routes based on a mock authentication state that persists in localStorage.",
    ],
    solutions: [
      "Used Angular Signals (`signal()`, `computed()`) as the primary tool for state management in services and components, providing a clear, derived, and reactive data flow.",
      "Created a single, smart HTTP Interceptor (`apiKeysInterceptor`) that conditionally attaches the correct API key (TMDB or Gemini) based on the outgoing request URL.",
      "Leveraged RxJS operators `debounceTime`, `distinctUntilChanged`, and `switchMap` on a Reactive Form's `valueChanges` observable to create an efficient, non-blocking search experience.",
      "Used RxJS `forkJoin` to parallelize multiple movie search requests (generated by the AI) and combined them into a single observable stream for the UI.",
      "Implemented functional `CanActivateFn` guards (`authGuard`, `publicGuard`) that check the `AuthService`'s `isLoggedIn` signal to protect routes and redirect users as needed.",
    ],
  },
  {
    id: 6,
    ...
    description:
      "An end-to-end analytics dashboard that tracks product interactions, aggregates analytics data, and visualizes results through an interactive React interface. Features JWT authentication, real-time filters, and comprehensive data visualization with PostgreSQL backend.",
    longDescription:
      "The Interactive Product Analytics Dashboard is a modern full-stack application designed to provide comprehensive product analytics and user interaction tracking. Built with React 19 for the frontend and FastAPI for the backend, the application serves as a complete analytics platform for businesses to understand user behavior and product performance. The system authenticates users with JWT tokens, tracks granular dashboard interactions such as filter usage and chart clicks, and aggregates product analytics by feature and over time. Users can filter data by date range, age group, gender, and feature while filters persist automatically, allowing seamless session continuity. The application features a clean, feature-based architecture on the frontend with Zustand for local state management and TanStack Query for intelligent server state caching. The backend implements a robust router-service-repository pattern with SQLAlchemy async for non-blocking database operations and Alembic for reproducible schema migrations.",
    ...
    links: {
      github:
        "https://github.com/hussein1574/Interactive-Product-Analytics-Dashboard",
      live: "https://dashboard.projectsfair.me/",
    },
    technologies: {
      frontend: [
        "React 19",
    ...
    features: [
      ...
      "Dashboard Interaction Tracking (Filter Usage, Chart Clicks)",
      "Product Analytics Aggregation by Feature",
      "Time-Based Analytics (Daily, Weekly, Monthly Trends)",
      "Multi-Field Filtering (Date Range, Age Group, Gender, Feature)",
      "Persistent Filter State with Cookies",
      "Real-Time Chart Updates with Recharts",
      "KPI Cards with Analytics Summary",
      "User Profile Management",
      "Responsive Design with Tailwind CSS",
      "Server State Management with TanStack Query",
      "Local UI State Management with Zustand",
      "Comprehensive Backend API Documentation (Swagger/OpenAPI)",
      "Database Seeding with Demo Data",
      "Docker Containerization for Easy Deployment",
      "nginx Reverse Proxy for Frontend-Backend Communication",
      "Async Database Operations for Performance",
      "Normalized Analytics Response Format",
    ],
    challenges: [
      "Separating concerns between complex interactive UI behavior and aggregation-heavy data logic across frontend and backend",
      "Implementing efficient caching and refetching strategies for analytics data that changes frequently",
      "Managing local filter state that needs to persist across page refreshes without complex backend workflows",
      "Handling large volumes of interaction events efficiently without overwhelming the database",
      "Designing async database queries that aggregate data quickly despite complex joins and grouping requirements",
      "Maintaining clear separation between request handling, business rules, and database logic in the backend",
      "Orchestrating multiple API calls on the frontend while maintaining proper loading and error states",
    ],
    solutions: [
      "Implemented explicit feature-based architecture where dashboard and auth code stay grouped by responsibility, reducing file-hopping and creating more contained changes",
      "Used TanStack Query for server state to provide automatic caching, background refetching, loading states, and error handling without implementing custom solutions",
      "Persisted filters in cookies managed by Zustand, keeping client-side state separate from server data and avoiding unnecessary backend storage logic",
      "Designed the backend with a clear router-service-repository flow where routers handle HTTP concerns, services hold business rules, and repositories isolate aggregation queries",
      "Leveraged SQLAlchemy async for non-blocking database operations that align with FastAPI's async nature, improving throughput for I/O-bound analytics queries",
      "Normalized the analytics response format so presentation components stay simple and avoid re-implementing transformation logic individually",
      "Implemented Alembic migrations to ensure schema changes remain reproducible across development, Docker, and production environments without manual setup",
    ],
  },
];
